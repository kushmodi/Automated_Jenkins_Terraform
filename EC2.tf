...
resource "null_resource" "slave-config" {
  ...
  provisioner "local-exec" {
    command = <<EOT
     sudo chown jenkins:jenkins ~/POC/node.xml
     ...
     sudo cat ~/POC/node.xml | java -jar /var/lib/jenkins/jenkins-cli.jar -s http://localhost:8080 -auth ${var.username}:${var.password} create-node ${aws_instance.ec2.tags.Name}
    EOT
  }
}
